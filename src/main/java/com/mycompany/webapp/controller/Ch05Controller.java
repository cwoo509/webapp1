...
public class Ch05Controller {
	...
	public String method1(@RequestHeader("User-Agent") String userAgent) {
		...
		if(userAgent.contains("Edg")) {
			logger.info("브라우져의 종류:엣지");
		}else if(userAgent.contains("Chrome")) {
			logger.info("브라우져의 종류:크롬");
		}else if(userAgent.contains("Trident/7.0")) {
			logger.info("브라우져의 종류:인터넷 익스플로러");
		}else if(userAgent.contains("MSIE")) {
			logger.info("브라우져의 종류:인터넷 익스플로러 10 이하");
	}return "ch05/content";
	...
	public String method3(HttpServletRequest request,
		...
		//쿠키를 응답 헤더에 포함해서 클라이언트(브라우져)에 보낸다
	...
	public String method4(HttpServletRequest request,
							HttpServletResponse response) {
		//브라우져가 WAS에 헤더에 보낸 쿠키 값을 읽기
		//getCookies:쿠키의 배열을 리턴함
	...
	public String method5(
		...
		//브라우져가 WAS에 헤더에 보낸 쿠키 값을 읽기
			logger.info(memberId + ":" + loginStatus);
		//응답 본문을 생성하는 뷰 리턴
		return "ch05/content";
}
}